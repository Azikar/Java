...
public class A_EkranoBandymas{
    ...
    void demoĮvadinis(){
        ...
        ScreenKTU sc1 = new ScreenKTU(ScreenKTU.Grid.OFF);
    ...
    void demoSpalvųPaletė(){
        ...
        scr.clearAll(blue);
        Color[] spalvos = {black, darkGray, gray, lightGray, blue, magenta, 
                            cyan, green, red, yellow, orange, pink, white};
        String[] spVardai = {"black", "darkGray", "gray ", "lightGray", "blue", "magenta",
                   "cyan  ", "green ", "red   ", "yellow", "orange", "pink  ", "white "};
        for (int i=0; i<spalvos.length; i++) {
            scr.setColors(spalvos[i], spalvos[(i+2) % spalvos.length]);
            scr.print(i+1, 2, ""+i);
    ...
    void demoĮstrižainės(){
        int n = 100; // eilučių ir stulpelių skaičius ekrane 
